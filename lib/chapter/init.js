  ...
  run: function (input, remote) {
    ...
    function ensureChapterJson (remote, input) {
      ...
      return require('../util/json').read(jsonPath, true)
        ...
        .then(function (json) {
          if (!json.name) {
            return inquire({name: 'name', message: 'There is no "name" in the chapter.json. What name should we use?', default: proposedName, type: 'input'})
        ...
        .then(function (json) {
          ...
          if (!json.location.name) {
            return inquire({name: 'location_name', message: 'There is no "location.name" in the chapter.json. What name should we use?', default: proposedName, type: 'input'})
              .then(function (result) {
          ...
          if (!json.location.country && json.location.name) {
            return require('../util/geocode').country(json.location.name, 'current chapter.json')
              .then(function (country) {
                return inquire({name: 'ok', message: json.location.name + ' seems to be in "' + country.long_name + '". Should "' + country.short_name + '"" be used as country?', type: 'confirm'})
                  .then(function (result) {
                    if (result.ok) {
                      json.location.country = country.short_name
                      commitLog.push('Added `location.country` to `chapter.json` through google api result.')
                    } else {
                      return inquire({name: 'ok', message: 'What country should we use then? \n Type the 2 letter code if you know it. \n If you are not sure press <Intro> and we\'ll help you.', type: 'input'})
                      .then(function (result) {
                        if (result.ok === '') {
                          var continents = require('../../country_designations.js').continents;
                          return inquire({name: 'continent', message: 'What continent is your country on?', type: 'list', choices: continents})
                            .then(function (result) {
                              var prettyCountriesListByContinent = require('../../country_designations.js').prettyCountriesListByContinent;
                              return inquire({name: 'composedCountryName', message: result.continent + '.\n What choose your country', type: 'list', choices: prettyCountriesListByContinent(result.continent)})
                                .then(function (result) {
                                  var country_code = result.composedCountryName.split(' - ')[0];
                                  json.location.country = country.country_code;
                                  return json
                                })
                            })
                        } else if(result.ok.length === 2) {
                          json.location.country = result.ok
                        }
                        console.log(result);
                      });
                    }
        ...
        .then(function (json) {
          if (!json.twitter) {
            ...
            return inquire({name: 'hashtag', message: 'There is no "twitter" hashtag in the chapter.json. What tag should we use?', default: proposedHashtag, type: 'input'})
              .then(function (result) {
                if (result.hashtag) {
        ...
        .then(function (json) {
          if (json.location && json.location.name && json.location.country && !(json.location.lat || json.location.lng)) {
            ...
            return require('../util/geocode')
              ...
              .then(function (geo) {
                return inquire({name: 'ok', message: 'We found the position ' + geo.lat + '/' + geo.lng + ' (lat/lng) for the location ' + location + '. Is this position okay?', type: 'confirm'})
                  .then(function (result) {
                    if (result.ok) {
    ...
    return inquire({name: 'ok', message: 'Should we start?', type: 'confirm'})
      .then(function (result) {
        if (!result.ok) {
          return Promise.reject({
            type: 'user',
