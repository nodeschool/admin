module.exports = {
  ...
  , run: function (input, remote, log) {
    ...
    function ensureEventsJson (input) {
      ...
      return require('../util/json').read(eventsJsonPath).then(function (json) {
        if (!json) {
    ...
    function ensureChapterJson (remote, input) {
      ...
      return require('../util/json').read(jsonPath)
        .then(function (json) {
          if (!json) {
            return require('../util/json').fromUrl(originalChapterUrl(chapter))
        ...
        .then(function (json) {
          if (!json.location.country && json.location.name) {
            return new Promise(function (resolve, reject) {
                ...
                log.info('json', 'The current chapter.json is missing a country specification. Looking up ' + json.location.name + ' for a country.')
                nodeGeo.geocode(json.location.name, function (err, data) {
                  if (err) {
                    return reject({
                      type: 'geojson',
                      message: err
        ...
        .then(function (json) {
          if (json.location && json.location.name && json.location.country && !(json.location.lat || json.location.lng)) {
            ...
            log.info('json', 'The current location doesnt have lat/lng data so the data is being fetched for location: ' + location)
            return new Promise(function (resolve, reject) {
                var nodeGeo = require('node-geo')
                nodeGeo.geocode(location, function (err, data) {
                  log.verbose('geocode', data)
                  return resolve((data && data.results && data.results[0] && data.results[0].geometry && data.results[0].geometry.location) || {})
                })    
              })
              .then(function (geo) {
                if (geo) {
