    ...
    return require('../util/repo').clean(remote, input.branch, [])
      ...
      .then(function () {
        ...
        .then(function (eventInput) {
          return {
            event: eventHelper.generateEvent(eventInput),
            name: eventHelper.generateEventName(eventInput)
          }
        })
      })
      .then(function (event) {
        ...
        var eventsPath = path.resolve(input.path, 'events', event.name)
        // save a file and then validate it
        return require('../util/fs').save(eventsPath, JSON.stringify(event.event, null, 2))
                  .then(function () {
                    return eventHelper.validateEvent(input.path, event.name)
                  })
      })
  }
}
