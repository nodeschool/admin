...
function ensureUpToDateBranch(remote, branchName) {
  ...
  return getStatus(remote, branchName).then(function (remoteRes) {
    ...
    if (message) {
      return inquire({name: 'track', message: message, type: 'confirm'})
    ...
    var state = remoteRes[4]
    if (/behind/.test(state)) {
      return inquire({name: 'track', message: 'The repo seems to be behind. Yes, to pull latest data!', type: 'confirm'})
        .then(function (result) {
          if (!result.track) {
            return Promise.reject({
              type: 'git',
...
function switchBranch(remote, branchName) {
  ...
  return repo.checkoutBranch(branchName)
    .catch(function (e) {
      ...
      return inquire({name: 'create', message: 'The required branch ' + branchName + ' is missing. Create it?', type: 'confirm'})
        .then(function (result) {
            if (!result.create) {
              return Promise.reject({
                type: 'git',
...
function ensureClean(repo, ignoredFiles) {
  return repo.getStatus().then(function (status) {
    ...
    if (status.length > 0) {
      ...
      return inquire({name: 'stash', message: 'This path contains uncommited changes.\n' + path.dirname(repo.path()) + '\n-> Should the changes be stashed?', type: 'confirm'})
        .then(function (result) {
          if (!result.stash) {
            return Promise.reject({
              type: 'git',
    ...
    if (status && status.length > 0) {
      ...
      repo.openIndex().then(function (index) {
        ...
        return index.addAll().then(function () {
          ...
          return inquire({name: 'ok', message: 'Should the commit be pushed?', type: 'confirm'})
        }).then(function (result) {
          if (result.ok) {
