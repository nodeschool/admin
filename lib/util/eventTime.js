...
parseDateTime.forEvent = function (event) {
  var start = parseDateTime(event.openDate, event.openTime, event.location.timeZone)
  var end = parseDateTime(event.endDate || event.openDate, event.endTime, event.location.timeZone)
  return {
    start: start,
    end: end,
