...
function askForOpenTime (input, suggestions) {
  return {
    name: 'openTime',
    message: 'What time will it start? (Format: HH:MM)',
    type: 'input',
    default: function (result) {
...
function askForEndTime () {
  return {
    name: 'endTime',
    message: 'What time will it end? (Format: HH:MM)',
    type: 'input',
    default: function (result) {
...
function generateEventName (event) {
  var date = generateDate(event.openDate)
  var time = generateTime(event.openTime)
  var name = event.name.split(' ').join('-')
  return  date + '_' + time + '_' + name + '.json'
}
